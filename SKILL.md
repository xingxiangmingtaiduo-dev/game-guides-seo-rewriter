...
- Give every localized version a language-specific Slug suffix so versions of the same article cannot collide: Simplified Chinese `-cn`, Traditional Chinese `-tw`, English `-en`, Portuguese `-pt`, Spanish `-es`, Thai `-th`, Indonesian `-id`, or Vietnamese `-vi`. The public DOCX and Markdown builders normalize the Slug to the matching suffix; write it correctly in the article package as well so validation passes.
